...
The system itself lives in the `/root/sass` directory as a collection of Sass partials broken up by scope and hierarchy. The partials get compiled down to singular CSS files that live in the main Root directory and cascade downward like so:
...
**Editor** 
- `/root/variables-editor.css`
- `/root/style-editor.css`
...
2. `cd` into the `wp-content/themes/root` directory and run `npm install`.
3. Next, run `npm build child-theme` and go through the prompts. This will create a fresh Root-based child-theme using the name you chose in the prompts (*The build tool is a work in progress. Follow the manual directions below to complete this step).
...
7. Continue to add and/or remove variables based on the [full list of available variables in Root](https://github.com/Automattic/themes-workspace/blob/add/variatheme/root/variables.css).
...
To use the system, simply duplicate the `root-child` theme directory and rename it `my-theme-name`. You’ll also want to do a search for `varia-child` strings and replace them with `my-theme-name` as follows:

- Search for: `'root-child'` and replace with: `'my-theme-name'` (with quotes).
- Search for: `root_child_` and replace with: `my_theme_name_`.
...
Use this simple Root child-theme example to familiarize yourself with the CSS-variables and to see how they influence the theme design when you change them. To start, open up the `/root-child-simple/variables.css` file. Change any of the values, save the file, and refresh the frontend of your site to see the changes. To add more variables, view the [full list of available variables](https://github.com/Automattic/themes-workspace/blob/add/variatheme/root/variables.css) from the Root parent them and copy any variables you want in to your child theme. 
The stylesheet structure for a child-theme cascades downward like this:

### Frontend
- `/root/variables.css`
- `/root-child-simple/variables.css` (System overrides)
- `/root/style.css`
- `/root-child-simple/style.css` (Extra CSS)

### Editor
- `/root/variables-editor.css`
- `/root-child-simple/variables-editor.css` (System overrides)
- `/root/style-editor.css`
- `/root-child-simple/style-editor.css` (Extra CSS)
...
**Root parent theme font style** (`/root/variables.css`)

`:root { --global--font-family: sans-serif; }`

**Root child-theme font-style overrides** (`/root-child/variables.css`)

`:root { --global--font-family: "Futura", "Helvetica", sans-serif; }`

### Extra CSS

These should be supplemental styles that give the theme a unique appearance beyond what’s included with the Root system defaults. Need to add a fixed header or add a box-shadow to your theme’s buttons? This is where those styles would go. When possible and appropriate, try to use Root CSS-variables in this stylesheet so that the system retains its usefulness across the theme.

## Advanced child-theme structure 
(See: `/root-child-advanced`)

The Root Child Advanced theme is an example of what a theme in production might look like using the Root system. It primarily relies on CSS-variables like the simple version but it also takes advantage of other child theming techniques. 

- A custom header and footer design by replacing the `header.php` and `footer.php` template parts.
- Some examples of custom block styles which will likely be common in many Root-based child-themes.
...
Customizing the responsive content-width of a Root Child Theme is also just as simple as changing a few variables. However, we can’t use CSS-variables directly in `@media` queries ([see here](https://www.w3.org/TR/css-variables-1/#using-variables)). Instead, Root child-themes come with a `responsive.scss` file that overwrites _only_ the responsive styles of the Root parent theme when compiled. 
...
Once you’ve set these variables to your liking in `/root-child-advanced/responsive.scss`, run `npm run build` on the child-theme which will recompile `responsive.scss` and reset the CSS-variables for content-width in `responsive.css`.
...
- Audit Root system for excessive overridden rules
